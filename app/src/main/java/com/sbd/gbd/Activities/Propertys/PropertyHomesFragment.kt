...
class PropertyHomesFragment : Fragment() {
    ...
    private fun fetchproducts(type : String) {
        var coroselimage = if(type == "" || type == null ) {
            FirebaseDatabase.getInstance().reference.child(AppConstants.products).orderByChild(
                AppConstants.Status).equalTo(AppConstants.user)
        } else {
            FirebaseDatabase.getInstance().reference.child(AppConstants.products).orderByChild(
                AppConstants.type).equalTo(type)
        }
